...
var initialElements  = function(canvas_info) {
    var buildImagesMemes = '<div hidden class="view-images-memes-ar-amm">';
    ...
    <div hidden class="wrapper-memes-preview-operations">
    ...
    <form hidden class="form-generat-meme" action="meme-generator-callback" method="post" enctype="multipart/form-data">
    ...
    `+buildImagesMemes+`
     ...
     <form hidden action="/" method="post" class="advanced-options-operations-form" accept-charset="UTF-8"><div><input type="hidden" name="form_build_id" value="form-C56rQ956oZXaasEGaFTd4nA7cf4GnkPvYCU0HEoO-fg">
...
$(this).each(function (delta) {
     ...
     var canvas_info = {
        ...
        _maxWidth: 650,//The initial size that used for scale
        _maxHeight: 700,
        position_left: 0,
        ratio_image: 1,
        ratio_width: 0,
...
$(document).find(".generate_meme").click(function(event){
...
//      memeGeneratorData.memeGenerator = canvas_info.getImageData(0, 0, canvas_info.final_width, canvas_info.final_height);
    ...
    $.each(texts.keys, function(index, id) {
        ...
        $('#ecard-form').append('<input type="hidden" name="artwork_base64" value="'+file+'">')
        $('#ecard-form').append('<input type="hidden" name="base" value="true">')
...
//              console.log(["imagesid", id, image_object.img.src]);
    ...
    //to make it above//
    canvas_info = MemeGenerator.get_canvas_info();
    var img = new Image();
    img.src = canvas_info.templates[0].url
